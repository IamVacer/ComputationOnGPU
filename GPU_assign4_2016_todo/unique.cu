	...
	int numElems)
{
	...
	if (tid < numElems) {
		...
		__syncthreads();
		if (tid == 0) {// if first index of entire input array, always return 1 because it is always unique
			outSelectionArray[tid] = 1;
		}
		else { //checking is needed
			if (tx == 0) {// if its the first element of the block, access to global input variable must be done
				if (sharedBlk[tx] > inSortedArray[tid - 1]) { //If there is an increment in values i.e. if unique
					outSelectionArray[tid] = 1;
				}
				...
				if (sharedBlk[tx] > sharedBlk[tx - 1]) { //If there is an increment in values i.e. if unique
					outSelectionArray[tid] = 1;
				}
				else { //It is a duplicate
					outSelectionArray[tid] = 0;
